...
// Wargame's typert remote .d.ts self-references the package's own `./types`
// subpath, which trips rolldown's cross-package resolve into shipping an
// empty placeholder for the imported contribution. Inlining the generated JS
// keeps the wire data identical without going through a subpath the resolver
// misreads. The path is intentionally a relative file import so the bundle
// tracks the bytes through rolldown's own pipeline.
import wargamePlatformRemote from './_inline/wargame-remote.js'
...
export type {} from '@deepseek-ai/dsh-wargame-platform/remote'
export type * from '@deepseek-ai/dsh-wargame-platform/types'
...
export async function apply(ctx: Context): Promise<() => Promise<void>> {
  ...
  try {
    for (const contribution of [
      ...
      subagentsRemote, sessionRemote, workspaceRemote, workspaceFilesRemote, wargamePlatformRemote,
    ]) {
